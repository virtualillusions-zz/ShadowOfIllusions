...
/**
 * Maintains and Handles the Graphical Interface for Life and Focus
 * Representation as well as current MoveSet and Queued
 *
...
public class GuiEssenceController implements ScreenController {
    ...
    /**
     * Sets the GUI's life visual parameters The life given should always be
     * based from the 100 percentile
     *
     * @param life the life
     */
     ...
     */
    public void setFocus(int currentFocus, int totalFocus) {
        focusText.getRenderer(TextRenderer.class).setText(currentFocus + " / " + totalFocus);
    }
    ...
    public void setRemainingCards(int remainingCards) {
        cardsLeftText.getRenderer(TextRenderer.class).setText("" + remainingCards);
    }
     ...
     */
    public void setEmblemUp(CardTrait type) {
        setElement("emblemUp", "Emblem", type.toString() + "Up");
    }
     ...
     * @param type the CardType
     */
    public void setEmblemDown(CardTrait type) {
        setElement("emblemDown", "Emblem", type.toString() + "Down");
    }
     ...
     * @param type the CardType
     */
    public void setEmblemLeft(CardTrait type) {
        setElement("emblemLeft", "Emblem", type.toString() + "Left");
    }
     ...
     * @param type the CardType
     */
    public void setEmblemRight(CardTrait type) {
        setElement("emblemRight", "Emblem", type.toString() + "Right");
    }

    /**
     * Removes the current Emblem from the top position
     */
    public void removeEmblemUp() {
        hideElement("emblemUp", true);
    }

    /**
     * Removes the current Emblem from the bottom position
     */
    public void removeEmblemDown() {
        hideElement("emblemDown", true);
    }

    /**
     * Removes the current Emblem from the left position
     */
    public void removeEmblemLeft() {
        hideElement("emblemLeft", true);
    }

    /**
     * Removes the current Emblem from the right position
     */
    public void removeEmblemRight() {
        hideElement("emblemRight", true);
    }

    /**
     * This sets the queued card lists in bulk for the player's GUI. Their
     * should only ever be three queued cards
     *
     * @param queuedCards an array of three queued cards
     */
    public void setQueued(CardTrait queuedCards[]) {
        for (int i = 0; i < 3; i++) {
            if (queuedCards[i] == null) {
                hideElement("queued" + (i + 1), true);                              //if array position null hide what ever previous image was in its place
            } else {
                setElement("queued" + (i + 1), "Queued", "queued" + queuedCards[i].toString() + (i + 1));
            }  //sets the queued positions image
        }
    }
    /*
    ...
    private void setElement(String partName, String folder, String filename) {
        ...
        NiftyImage newImage = nifty.getRenderEngine().createImage(screen, "Interface/EssenceInterface/" + folder + "/" + filename + ".png", false); // false means don't linear filter the image, true would apply linear filtering
    ...
    private void hideElement(String elementID, boolean hide) {
        ...
        if (hide) {
            element.hide();
        } //hides the element if the boolean says to
        else {
            element.show();
        }       //shows the element if the boolean says to
    }
    ...
    private void setGUILife(int life) {
        ...
        for (int i = 1; i <= 5; i++) {
            if (life / 20 + 1 >= i) {
                hideElement("mainLife" + i, false);
            } else {
                hideElement("mainLife" + i, true);
            }
            //Sets the sub Life Bars
            if (i < 5) {
                if ((((life) % 20) / 5) + 1 >= i) {
                    hideElement("subLife" + i, false);
                } else {
                    hideElement("subLife" + i, true);
                }
            }
        }
    }
    ...
    /**
     * This is the method that should be called directly to set up the GUI
     * portion of the EssenceControls TODO: Fix GUI to be dynamic for now will
     * be static
     *
     * @param spatial
