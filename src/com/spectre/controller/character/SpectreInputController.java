...
public class SpectreInputController extends SpectreAbstractController implements ActionListener, AnalogListener {
    ...
    public void onAction(String name, boolean isPressed, float tpf) {        
        ...
        } else if (name.equals(getPlayerName() +":"+controlInputs.Action7)) {//CM: GatherFocus(whileStill) __ AM: Perform Special Maneuver
        ...
        } else if (name.equals(getPlayerName() + ":"+controlInputs.LeftTrigger)) {// Lock On Hold:Zoom View
            //will require use of SpectreCameraController Directly
            if (isPressed == true) {
            }
    ...
    public void onAnalog(String name, float value, float tpf) {
        ...
        if (name.equals(getPlayerName() + ":"+controlInputs.LeftThumbstickUp)) {//Move Character Up   
            ...
            getCameraCont().inputRotateCamera(value);
        } else if (name.equals(getPlayerName() + ":"+controlInputs.RightThumbstickLeft)) {//Rotate Camera Left
            getCameraCont().inputRotateCamera(-value);
        } else if (name.equals(getPlayerName() + ":ZoomIn")) {//Rotate Camera Left
            getCameraCont().inputZoomCamera(value, true);
        } else if (name.equals(getPlayerName() + ":ZoomOut")) {//Rotate Camera Left
            getCameraCont().inputZoomCamera(value, false);
        }
    }
